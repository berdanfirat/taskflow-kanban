...
export function TaskCard({ task, onDeleteTask }: any) {
  ...
  // Sürüklerken arkada bırakılan gölge/hayalet görünüm
  if (isDragging) {
  ...
  // Varsayılan veriler (Eğer veritabanında yoksa boş durmasın diye)
  const priority = task.priority || 'Orta';
  const dueDate = task.due_date || 'Belirtilmedi';
  const assignee = task.assignee || 'BF'; // Berdan Fırat baş harfleri
  ...
  return (
    <div 
      ...
      {...attributes} 
      {...listeners} 
      // MOBİL DÜZELTME: touch-none (kaydırmayı engeller), select-none (metin seçimini engeller)
    ...
    >
      {/* Üst Kısım: Etiketler ve Silme Butonu */}
      <div className="flex justify-between items-start mb-2">
        <div className="flex gap-2">
           {/* Dinamik Öncelik Etiketi */}
      ...
      {/* Alt Kısım: Tarih ve Sorumlu Kişi */}
